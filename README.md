...
```python
from data import extract\_zips, flatten, split\_dataset

extract\_zips()      # zip → data/raw/
flatten()           # data/raw/ → data/flat/  (음식별 폴더로 평탄화)
split\_dataset()     # data/flat/ → data/split/{train,val,test}/
```
...
## !\[training curves](docs/training\_curves.png)
...
## !\[confusion matrix](docs/confusion\_matrix.png)
...
## AI Hub 데이터는 비상업적 연구용. 학습된 모델 배포 시 라이선스 확인 필요.라이선스
